...
fn sys_mmap(
    ...
    let addr_space = curr_task.as_task_ref().task_ext().aspace.clone();
    let mut addr_space = addr_space.lock();

    let file = match get_file_like(fd) {
        Ok(file) => file,
        Err(e) => {
            info!("mmap err in get_file_like: {}", e.as_str());
            return -e.code() as isize;
        }
    };
    let mut buf = vec![0; length];
    if let Err(e) = file.read(&mut buf) {
        info!("mmap err in read: {}", e.as_str());
        return -e.code() as isize;
    }

    let Some(_mmap_flags) = MmapFlags::from_bits(flags) else {
        info!("mmap err: invalid flags");
        return -LinuxError::EINVAL.code() as isize;
    };
    let Some(mapping_flags) = MappingFlags::from_bits(prot as usize) else {
        info!("mmap err: invalid prot");
        return -LinuxError::EINVAL.code() as isize;
    };
    let mapping_flags = mapping_flags | MappingFlags::USER | MappingFlags::EXECUTE;

    let addr = (addr as usize).into();
    let length = length.align_up_4k();
    let limit = VirtAddrRange::from_start_size(addr_space.base(), addr_space.size());
    let Some(addr) = addr_space.find_free_area(addr, length, limit) else {
        info!("mmap err in find_free_area");
        return -LinuxError::ENOMEM.code() as isize;
    };
    match addr_space.map_alloc(addr, length, mapping_flags, true) {
        Ok(_) => (),
        Err(e) => {
            info!("mmap err in map_alloc: {}", e.as_str());
            return -LinuxError::from(e).code() as isize;
        }
    }
    if let Err(e) = addr_space.write(addr, &buf) {
        info!("mmap err in write: {}", e.as_str());
        return -LinuxError::from(e).code() as isize;
    }
